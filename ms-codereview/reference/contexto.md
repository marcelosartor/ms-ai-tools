...
O script fala com o tracker configurado — ClickUp, Jira, Linear ou GitHub
Issues (este último sem variável de `.env`: usa o `gh` já autenticado, e só
entra como candidato automático se nenhum dos outros estiver configurado)
— e grava o
...
- `deps`: major bump tem changelog/breaking lido e citado; lockfile bate
  ...
  encontrado — em projeto Node, `checklists/backend-node.md`, grupo
  "Configuração e segurança" de `## Comum`, cobre a mesma pergunta em
  detalhe.
