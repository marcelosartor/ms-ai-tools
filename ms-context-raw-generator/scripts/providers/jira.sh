#!/usr/bin/env bash
#
# Provider Jira para fetch-raw-context.sh. Mesmo contrato do clickup.sh.
#
...
jira_attachments() {
  # o download de fields.attachment[].content exige a mesma credencial da
  # API (Basic/Bearer), por isso auth:true.
  jq '[ (.fields.attachment // [])[] | {
    id: (.id | tostring),
    titulo: (.filename // (.id | tostring) // "anexo"),
    extensao: ((.filename // "") | if test("\\.") then (split(".") | last | ascii_downcase) else "" end),
    tamanho: (.size // 0),
    data: (.created // null),
    autor: (.author.displayName // null),
    url: (.content // null),
    auth: true
